...
async function main() {
    ...
    const resp = await fetch(OVERPASS, { method: 'POST', body: query, headers: { 'Content-Type': 'text/plain' } });
    ...
    const transitNodes = data.elements.filter((el) => el.type === 'node' && (el.tags?.highway === 'bus_stop' || el.tags?.public_transport === 'platform' || el.tags?.railway === 'station'));
    const elevatorNodes = data.elements.filter((el) => el.type === 'node' && (el.tags?.elevator || el.tags?.['access:wheelchair']));
    const handrailNodes = data.elements.filter((el) => el.type === 'node' && (el.tags?.handrail || el.tags?.guardrail || el.tags?.barrier === 'handrail'));
    ...
    function mathAsinSafe(x) { return Math.asin(Math.min(1, Math.max(-1, x))); }
    for (const el of data.elements) {
        if (el.type !== 'way')
            continue;
        const tags = el.tags || {};
        ...
        for (let i = 1; i < coords.length; i++) {
            ...
            const transitNearby = transitNodes.some((tn) => dist([tn.lon, tn.lat], mid) < 50);
            const elevatorNearby = elevatorNodes.some((en) => dist([en.lon, en.lat], mid) < 30);
            const handrailNearby = handrailNodes.some((hn) => dist([hn.lon, hn.lat], mid) < 20);
