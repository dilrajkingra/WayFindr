...
import { Coord, Edge } from '../types.js';

export function haversine(a: Coord, b: Coord){
...
export function snapToGraphPoint(p: Coord, edges: Edge[]){
  // Find nearest node from all edges (check all coordinates, not just endpoints)
  let best: { edge: Edge|null, dist: number, coord: Coord, atNode:number|null } = { edge:null, dist: 1e12, coord: p, atNode: null };
  
  // Track all unique nodes we've seen
  const nodeCoords = new Map<number, Coord>();
  ...
  edges.forEach(e=>{
    ...
    e.coords.forEach((coord, idx) => {
      ...
      if (dist < best.dist) {
        ...
        if (idx === 0) nodeId = e.from;
        else if (idx === e.coords.length - 1) nodeId = e.to;
        else {
