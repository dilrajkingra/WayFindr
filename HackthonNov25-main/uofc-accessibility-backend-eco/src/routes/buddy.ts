...
r.post('/location', async (req: Request, res: Response) => {
...
r.get('/nearby', async (req: Request, res: Response) => {
  const userId = (req as any).userId;
  ...
  const nearby = users
    ...
    .map(u => {
      const dist = haversine(
        [currentUser.lastLng!, currentUser.lastLat!],
        [u.lastLng!, u.lastLat!]
      );
...
r.get('/requests', async (req: Request, res: Response) => {
  const userId = (req as any).userId;
...
r.post('/request/:id/respond', async (req: Request, res: Response) => {
  const userId = (req as any).userId;
