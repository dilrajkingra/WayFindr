...
import { Coord, Edge, Graph } from '../types.js';

type Profile = 'default' | 'wheelchair' | 'visually_impaired' | 'eco';

const r = Router();

type Wx = { icy?: boolean; lowVis?: boolean; heat?: boolean };
function wxStub(): Wx { return {}; } // plug in OpenWeather flags if you wish

function isRough(e: Edge) {
...
function isGreenEdge(e: Edge) {
  const hw = (e as any).highway || '';
  const foot = (e as any).footway || '';
  return ['path','footway','pedestrian'].includes(hw) || foot === 'sidewalk';
}
function len(coords: Coord[]){ return lineLength(coords); }
...
function analyzeEdgeReasons(e: Edge, profile: Profile, wx: Wx): EdgeReason[] {
...
function costEdge(e: Edge, profile: Profile, wx: Wx){
  if ((e as any).__closed) return Infinity;
  ...
  if (profile === 'wheelchair'){
    if (e.is_stairs) return Infinity;
    ...
    // Slope handling
    const slope = e.incline_pct || 0;
    if (slope > 8) c *= 2.0; // Very steep slopes heavily penalized
    else if (slope > 5) c *= 1.5; // Moderate slopes penalized
    else if (slope > 0 && slope <= 3) c *= 0.95; // Gentle slopes slightly preferred
    if (wx.icy && slope > 3) c *= 1.6;
    
    // Accessibility features
    if (isRough(e)) c *= 1.25;
    if ((e as any).has_curb_ramp) c *= 0.85;
    if (e.has_elevator) c *= 0.8; // Elevator access greatly preferred
    if (e.has_handrail) c *= 0.9; // Handrails provide safety
    if (e.obstacle_free) c *= 0.9; // Clear paths preferred
  ...
  if (profile === 'visually_impaired'){
    ...
    // Navigation aids
    if (e.audio_beacon) c *= 0.75; // Audio beacons greatly help navigation
    if (e.high_contrast) c *= 0.85; // High contrast markings help visibility
    if (e.tactile_paving) c *= 0.9; // Tactile paving is very helpful
    if (e.has_handrail) c *= 0.9; // Handrails provide guidance
    if (e.obstacle_free) c *= 0.85; // Clear paths are much safer
    ...
    const turnC = (e.turn_complexity || 0);
    if (turnC > 3) c *= 1.3; // Very complex turns heavily penalized
    else if (turnC > 1) c *= 1.1; // Moderate complexity penalized
    else c *= 1 + 0.03 * turnC; // Simple turns slightly penalized
...
function heuristic(a: Coord, b: Coord){ return haversine(a,b); }

function aStar(g: Graph, startCoord: Coord, endCoord: Coord, profile: Profile){
  const wx = wxStub();
  ...
  const getCoordOfNode = (id: number)=> g.nodes.find(n=>n.id===id)?.coord as Coord;

  const open = new Set<number>([startNode]);
  const cameFrom = new Map<number, number>();
  const gScore = new Map<number, number>(); gScore.set(startNode, 0);
  const fScore = new Map<number, number>(); fScore.set(startNode, heuristic(getCoordOfNode(startNode), getCoordOfNode(endNode)));
  ...
  while (open.size && iterations < maxIterations){
    ...
    if (current === endNode){
      ...
      const pathNodes: number[] = [current];
      ...
      while (cameFrom.has(pathNodes[0])){
        ...
        if (edgeIdx !== undefined) pathEdges.unshift(edgeIdx);
        pathNodes.unshift(fromNode);
        prevNode = fromNode;
      }
      
      // Analyze why this path was chosen
      const pathReasons: EdgeReason[] = [];
      const reasonCounts: Record<string, { count: number; impact: 'positive' | 'negative' | 'neutral' }> = {};
      ...
      const coords: Coord[] = pathNodes.map(id => getCoordOfNode(id));
      return { 
        ok: true, 
        coords, 
...
function coordsLength(coords: Coord[]){
  let d = 0;
